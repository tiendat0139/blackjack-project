...
function Register({user}) {
    ...
    const handleSubmit = (e) => {
        ...
        if(Object.keys(validate(formValues)).length === 0 && isSubmitted){
            Axios.post('http://localhost:5000/register', {
                ...
                avatar: avatar.selectedFileImg
            }).then((response) => {
                if(response.data){
                    setIsRegister(true);
                } else {
                    setIsRegister(false);
                    setMessage('ユーザー名「' + formValues.username + '」はもう保存しています。');
                }
            });
        }
    }
    ...
    const validate = (values) => {
        ...
        if (!values.username){
            ...
            errors.username = "ユーザー名の長さは6から45まで";
        } else if (!regexUsername.test(values.username) && values.username.length > 0){
            errors.username = "無効なユーザー名";
        }
        ...
        if (!values.password){
            ...
            errors.password = "パスワードとパスワード確認は違っています";
        }
        ...
        if (!values.name){
            ...
            errors.name = "氏名の長さは４から３０まで";
        } else if (!regexName.test(values.name) && values.name.length > 0){
            errors.name = "無効な氏名";
        };
            ...
            <div className='register-card'>
                ...
                <div className='register-two-div'>
                    ...
                    <div className='register-div'>
                        ...
                        {formErrors.casinoName && <p className='text-sm text-red-600'>{formErrors.casinoName}</p>}
                        <div className="mb-3 auth-input-label">
                        ...
                        <div>
                            <AvatarUpload handleSubmit={getAvatar}/>
                        </div>
